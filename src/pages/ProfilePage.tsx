...
export default function ProfilePage() {
  ...
  const [profCoursesCount, setProfCoursesCount] = useState(0);
        ...
        try {
          ...
          const updatePromises = coursesSnap.docs.map(courseDoc => 
            updateDoc(doc(db, "courses", courseDoc.id), {
              professorName: editData.fullName,
              professorPhotoURL: finalPhotoURL
            })
          );
          await Promise.all(updatePromises);
        } catch (courseErr) {
          console.error("Error updating professor courses:", courseErr);
        }
      }
                ...
                {profile.role === 'professor' ? (
                  <>
                    <div className="space-y-2">
                      ...
                      <p className="text-3xl font-black text-[var(--text-main)]">{profCoursesCount}</p>
                    </div>
                    <div className="space-y-2">
                      <p className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest">Feedback Network</p>
                      <p className="text-3xl font-black text-indigo-600">{(profile.rating !== undefined && profile.rating !== null) ? profile.rating.toFixed(1) : '0.0'}</p>
                    </div>
                  </>
                ) : (
                  <>
